...
class sqlQueries {
    ...
    //method that updates a selected employees role_id in the employee database
    updateEmployee(role, id) {
        return this.db.promise().query(
            `UPDATE employee SET ? WHERE ?`, [role, id]
        );
    }

    //mehtod that displays a selected manager's direct reports
